...
def test_removed_then_rediscovered_appliance_reattaches_and_clears_flag():
    # The #15 recovery story: same-haId re-pair must clear the orphan flag and
    # resume state flow with no user action.
    p = _plugin()
    appliance = _appliance("HA-1", "Dishwasher", "Dishwasher")
    p._coordinator = _FakeCoord([appliance])
    dev = _device(12, "dishwasher", "HA-1")
    p.deviceStartComm(dev)
    p._appliance_removed("HA-1")
    assert dev.error_state is not None

    fresh = _appliance("HA-1", "Dishwasher", "Dishwasher")
    p._appliance_discovered(fresh)               # re-pair under the same haId
    assert p._bridges[dev.id].is_attached
    assert dev.error_state is None               # flag cleared by the attach push
    fresh.merge_items([{"key": OP, "value": "BSH.Common.EnumType.OperationState.Run"}])
    assert dev.states["operationState"] == "Run"
